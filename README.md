...
1. `currentPage` меньше установленного лимита `pagesLimit`: выводим страницы от первой до `pagesLimit`;
   ...
   > [!NOTE]
   > Если `pagesLimit = 11`, то выводятся первые 11 страниц, например.
...
2. `currentPage` больше или равно `pagesAmount - (pagesLimit - 2)`: выводим последний блок страниц от `pagesAmount - (pagesLimit - 1)` до последней страницы;
   ...
   > [!NOTE]
   > Если `pagesLimit = 11`, то выводятся последние 11 страниц, например.
...
3. Срединный вариант, когда `currentPage` выводится в центре, а по бокам помещаются страницы в количестве `pagesLimit - 1`. Алгоритм подстраивается под чётное и нечётное значение `pagesLimit`;
   ...
   > [!NOTE]
   > Если `pagesLimit = 11`, то начиная с 11-ой страницы `currentPage`выводится в центре, например.
